...
export async function handleWhatsappCommand(args: string[], runtime: RuntimeContext): Promise<unknown> {
  ...
  if (group === "templates" && action === "list") return listWaTemplates(deps, rest[0] ?? "", token);
  if (group === "templates" && action === "create") return createWaTemplate(deps, rest[0] ?? "", token, JSON.parse(rest[1] ?? "{}"));
  if (group === "phone-numbers" && action === "list") return listWaPhoneNumbers(deps, rest[0] ?? "", token);
