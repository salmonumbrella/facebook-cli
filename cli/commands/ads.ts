...
import {
  ...
  executeDeploy,
} from "../../src/domains/ads-deploy.js";
import {
  collectStats,
  analyzeStats,
  validateStats,
  exportStatsCsv,
} from "../../src/domains/ads-stats.js";
import {
...
export async function handleAdsCommand(args: string[], runtime: RuntimeContext): Promise<unknown> {
  ...
  if (group === "deploy") {
    ...
    if (runtime.dryRun || !token || !accountId)
      return { ok: true, dryRun: true, valid: validateDeployConfig(config) };
  ...
  if (group === "audience") {
    ...
    if (sub === "estimate-size") {
      return getAudienceSizeEstimate(
        d,
        rest[0] ?? "",
        requireToken(runtime),
        parseJsonArg(rest[1]),
      );
    }
    throw new Error(
      "Usage: fbcli ads audience <search-interests|search-behaviors|estimate-size> ...",
    );
  }
  ...
  if (group === "stats") {
    if (action === "collect") {
      ...
      if (!accountId || !since || !until)
        throw new Error(
          "Usage: fbcli ads stats collect <account-id> <since> <until> [storage-dir]",
        );
    ...
    if (action === "analyze")
      return analyzeStats((parseJsonArg(rest[0]).data as Array<Record<string, unknown>>) ?? []);
  ...
  if (group === "accounts") {
    if (action === "list")
      return runtime.dryRun ? { ok: true, route: "ads accounts list" } : listAdAccounts(d, token!);
    if (action === "get")
      return runtime.dryRun
        ? { ok: true, route: "ads accounts get" }
        : getAdAccount(d, rest[0] ?? "", token!);
  }

  if (group === "campaigns") {
    if (action === "list")
      return runtime.dryRun
        ? { ok: true, route: "ads campaigns list" }
        : listCampaigns(d, rest[0] ?? "", token!);
    if (action === "get")
      return runtime.dryRun
        ? { ok: true, route: "ads campaigns get" }
        : getCampaign(d, rest[0] ?? "", token!);
    if (action === "create")
      return runtime.dryRun
        ? { ok: true, route: "ads campaigns create" }
        : createCampaign(d, rest[0] ?? "", token!, parseJsonArg(rest[1]));
    if (action === "update")
      return runtime.dryRun
        ? { ok: true, route: "ads campaigns update" }
        : updateCampaign(d, rest[0] ?? "", token!, parseJsonArg(rest[1]));
    if (action === "pause")
      return runtime.dryRun
        ? { ok: true, route: "ads campaigns pause" }
        : pauseCampaign(d, rest[0] ?? "", token!);
    if (action === "activate")
      return runtime.dryRun
        ? { ok: true, route: "ads campaigns activate" }
        : activateCampaign(d, rest[0] ?? "", token!);
    if (action === "delete")
      return runtime.dryRun
        ? { ok: true, route: "ads campaigns delete" }
        : deleteCampaign(d, rest[0] ?? "", token!);
  }

  if (group === "adsets") {
    if (action === "list")
      return runtime.dryRun
        ? { ok: true, route: "ads adsets list" }
        : listAdSets(d, rest[0] ?? "", token!);
    if (action === "get")
      return runtime.dryRun
        ? { ok: true, route: "ads adsets get" }
        : getAdSet(d, rest[0] ?? "", token!);
    if (action === "create")
      return runtime.dryRun
        ? { ok: true, route: "ads adsets create" }
        : createAdSet(d, rest[0] ?? "", token!, parseJsonArg(rest[1]));
    if (action === "update")
      return runtime.dryRun
        ? { ok: true, route: "ads adsets update" }
        : updateAdSet(d, rest[0] ?? "", token!, parseJsonArg(rest[1]));
  }

  if (group === "ads") {
    if (action === "list")
      return runtime.dryRun
        ? { ok: true, route: "ads ads list" }
        : listAds(d, rest[0] ?? "", token!);
    if (action === "get")
      return runtime.dryRun ? { ok: true, route: "ads ads get" } : getAd(d, rest[0] ?? "", token!);
    if (action === "create")
      return runtime.dryRun
        ? { ok: true, route: "ads ads create" }
        : createAd(d, rest[0] ?? "", token!, parseJsonArg(rest[1]));
    if (action === "update")
      return runtime.dryRun
        ? { ok: true, route: "ads ads update" }
        : updateAd(d, rest[0] ?? "", token!, parseJsonArg(rest[1]));
  }

  if (group === "creatives") {
    if (action === "list")
      return runtime.dryRun
        ? { ok: true, route: "ads creatives list" }
        : listCreatives(d, rest[0] ?? "", token!);
    if (action === "get")
      return runtime.dryRun
        ? { ok: true, route: "ads creatives get" }
        : getCreative(d, rest[0] ?? "", token!);
    if (action === "create")
      return runtime.dryRun
        ? { ok: true, route: "ads creatives create" }
        : createCreative(d, rest[0] ?? "", token!, parseJsonArg(rest[1]));
  }

  if (group === "images" && action === "upload") {
    return runtime.dryRun
      ? { ok: true, route: "ads images upload" }
      : uploadImage(d, rest[0] ?? "", token!, parseJsonArg(rest[1]));
  }
  ...
  if (group === "insights" && action === "get") {
    ...
    if (params.breakdowns) {
      return runtime.dryRun
        ? { ok: true, route: "ads insights get", breakdowns: params.breakdowns }
        : getInsightsWithBreakdowns(d, rest[0] ?? "", token!, params);
    }
    return runtime.dryRun
      ? { ok: true, route: "ads insights get" }
      : getInsights(d, rest[0] ?? "", token!, params);
  }

  if (group === "audiences") {
    if (action === "list")
      return runtime.dryRun
        ? { ok: true, route: "ads audiences list" }
        : listAudiences(d, rest[0] ?? "", token!);
    if (action === "get")
      return runtime.dryRun
        ? { ok: true, route: "ads audiences get" }
        : getAudience(d, rest[0] ?? "", token!);
    if (action === "create")
      return runtime.dryRun
        ? { ok: true, route: "ads audiences create" }
        : createAudience(d, rest[0] ?? "", token!, parseJsonArg(rest[1]));
    if (action === "update")
      return runtime.dryRun
        ? { ok: true, route: "ads audiences update" }
        : updateAudience(d, rest[0] ?? "", token!, parseJsonArg(rest[1]));
    if (action === "delete")
      return runtime.dryRun
        ? { ok: true, route: "ads audiences delete" }
        : deleteAudience(d, rest[0] ?? "", token!);
  }
