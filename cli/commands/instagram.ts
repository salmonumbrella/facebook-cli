...
export async function handleInstagramCommand(args: string[], runtime: RuntimeContext): Promise<unknown> {
  const [group, action, ...rest] = args;
  if (!group) throw new Error("Usage: fbcli ig <accounts|media|account|comments|publish|stories> ...");
  ...
  if (group === "media" && action === "insights") return getIgMediaInsights(deps, rest[0] ?? "", token);
  if (group === "account" && action === "insights") return getIgAccountInsights(deps, rest[0] ?? "", token);
  if (group === "comments" && action === "list") return listIgComments(deps, rest[0] ?? "", token);
  if (group === "comments" && action === "reply") return replyIgComment(deps, rest[0] ?? "", token, rest.slice(1).join(" ") || "");
  if (group === "publish") return publishIgMedia(deps, action ?? "", token, { image_url: rest[0], caption: rest.slice(1).join(" ") });
