...
describe("audience targeting search", () => {
  it("searches interests via /search?type=adinterest", async () => {
    const graphApi = mock(async () => ({
      data: [{ id: "6003107902433", name: "Online shopping", audience_size: 910000000 }],
    }));
    await searchInterests({ graphApi } as any, "TOKEN", "shopping");
  ...
  it("estimates audience size via /act_{id}/delivery_estimate", async () => {
    ...
    await getAudienceSizeEstimate({ graphApi } as any, "123", "TOKEN", {
      age_min: 18,
      age_max: 35,
