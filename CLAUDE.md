...
go run ./cmd/fbcli posts <page_name>
```

Or build and install:

```bash
make build    # → bin/fbcli
make install  # → $GOPATH/bin/fbcli
```
...
Go CLI entrypoint is `cmd/fbcli/main.go`. Command handlers live in `internal/cmd/` with domain logic in `internal/facebook/` and supporting packages in `internal/api/`, `internal/auth/`, `internal/config/`, `internal/output/`, and `internal/profile/`.
